...
def get_order_line_payload(line: "OrderLine"):
    ...
    return {
        ...
        "unit_undiscounted_price_amount": quantize_price(line.undiscounted_unit_price.gross.amount, currency),
        "total_undiscounted_price_amount": quantize_price(
            line.undiscounted_unit_price.gross.amount * line.quantity, currency
        ),
...
def get_discount_amount(order):
    total = order.total_gross_amount - order.get_undiscounted_total().gross.amount - order.shipping_price_gross_amount
    order_discounts = order.discounts.all()
    voucher_discount = None
    for order_discount in order_discounts:
        if order_discount.type == OrderDiscountType.VOUCHER:
